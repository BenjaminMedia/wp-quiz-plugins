	...
	$(document).ready(function() 
	{
		...
		wpvqgr.getAppreciations = function(finalScore)
		{
			...
			$.each(finalScore, function (index, value) 
			{
				var p_id = value['p_id'];
				appreciations.push({
					'name': wpvqgr_quiz.appreciations[p_id].wpvqgr_quiz_appreciation_name,
					'content': wpvqgr_quiz.appreciations[p_id].wpvqgr_quiz_appreciation_content,
					'picture': wpvqgr_quiz.appreciations[p_id].wpvqgr_quiz_appreciation_picture,
					'percentage': Math.round(value.score * 100 / totalScore)
				});
			});
