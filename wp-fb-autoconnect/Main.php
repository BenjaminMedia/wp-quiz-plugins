...
function jfb_output_facebook_btn($callbackName=0)
{
    ...
    global $opt_jfb_ask_stream, $opt_jfbp_requirerealmail;
    ...
    $email_perms = get_option($opt_jfb_ask_perms_email) || get_option($opt_jfbp_requirerealmail);
    $books_perms = get_option($opt_jfb_ask_perms_books);
    $about_perms = get_option($opt_jfb_ask_perms_about);
    $fitness_perms = get_option($opt_jfb_ask_perms_fitness);
    $music_perms = get_option($opt_jfb_ask_perms_music);
    $news_perms = get_option($opt_jfb_ask_perms_news);
    $birthday_perms = get_option($opt_jfb_ask_perms_birthday);
    $education_perms = get_option($opt_jfb_ask_perms_education);
    $hometown_perms = get_option($opt_jfb_ask_perms_hometown);
    $photos_perms = get_option($opt_jfb_ask_perms_music);
    $posts_perms = get_option($opt_jfb_ask_perms_posts);
    $relationships_perms = get_option($opt_jfb_ask_perms_relationships);
    $relationship_details_perms = get_option($opt_jfb_ask_perms_relationship_details);
    $religion_politics_perms = get_option($opt_jfb_ask_perms_religion_politics);
    $places_perms = get_option($opt_jfb_ask_perms_places);
    $videos_perms = get_option($opt_jfb_ask_perms_videos);
    $adsread_perms = get_option($opt_jfb_ask_perms_adsread);
    $friends_perms = get_option($opt_jfb_ask_perms_friends);
    $videoactions_perms = get_option($opt_jfb_ask_perms_videoactions);
    $events_perms = get_option($opt_jfb_ask_perms_events);
    $games_perms = get_option($opt_jfb_ask_perms_games);
    $likes_perms = get_option($opt_jfb_ask_perms_likes);
    $location_perms = get_option($opt_jfb_ask_perms_location);
    $groups_perms = get_option($opt_jfb_ask_perms_groups);
    $websites_perms = get_option($opt_jfb_ask_perms_websites);
    $work_perms = get_option($opt_jfb_ask_perms_work);
    $customfreinds_perms = get_option($opt_jfb_ask_perms_customfreinds);
    
    if( $email_perms )                     $scope  = 'email';
    if( $about_perms )                     $scope .= ',user_about_me';
    if( $books_perms )                     $scope .= ',user_actions.books';
    if( $fitness_perms )                   $scope .= ',user_actions.fitness';
    if( $music_perms )                     $scope .= ',user_actions.music';
    if( $news_perms )                      $scope .= ',user_actions.news';
    if( $birthday_perms )                  $scope .= ',user_birthday';
    if( $education_perms )                 $scope .= ',user_education_history';
    if( $hometown_perms )                  $scope .= ',user_hometown';
    if( $photos_perms )                    $scope .= ',user_photos';
    if( $posts_perms )                     $scope .= ',user_posts';
    if( $relationships_perms )             $scope .= ',user_relationships';
    if( $relationship_details_perms )      $scope .= ',user_relationship_details';
    if( $religion_politics_perms )         $scope .= ',user_religion_politics';
    if( $places_perms )                    $scope .= ',user_tagged_places';
    if( $videos_perms )                    $scope .= ',user_videos';
    if( $adsread_perms )                   $scope .= ',ads_read';
    if( $friends_perms )                   $scope .= ',user_friends';
    if( $videoactions_perms )              $scope .= ',user_actions.video';
    if( $events_perms )                    $scope .= ',user_events';
    if( $games_perms )                     $scope .= ',user_games_activity';
    if( $likes_perms )                     $scope .= ',user_likes';
    if( $location_perms )                  $scope .= ',user_location';
    if( $groups_perms )                    $scope .= ',user_managed_groups';
    if( $websites_perms )                  $scope .= ',user_website';
    if( $work_perms )                      $scope .= ',user_work_history';
    if( $customfreinds_perms )             $scope .= ',read_custom_friendlists';
    if( $shortcode_perm != "") 
        { 
        $scope = $shortcode_perm;
        // echo "<script type='text/javascript'>alert('$scope');</script>"; 
        };
...
function jfb_output_facebook_callback($redirectTo=0, $callbackName=0)
{
     ...
     global $opt_jfb_ask_perms_email, $opt_jfb_ask_perms_about, $opt_jfb_ask_perms_books, $opt_jfb_ask_perms_fitness, $opt_jfb_ask_perms_music, $opt_jfb_ask_perms_news, $opt_jfb_ask_perms_birthday, $opt_jfb_ask_perms_education, $opt_jfb_ask_perms_hometown, $opt_jfb_ask_perms_photos, $opt_jfb_ask_perms_posts, $opt_jfb_ask_perms_relationsship, $opt_jfb_ask_perms_relationsshipdetails, $opt_jfb_ask_perms_religionpolitics, $opt_jfb_ask_perms_videos, $opt_jfb_ask_perms_adsread, $opt_jfb_ask_perms_places, $opt_jfb_ask_perms_friends, $opt_jfb_ask_perms_videoactions, $opt_jfb_ask_perms_events, $opt_jfb_ask_perms_games, $opt_jfb_ask_perms_likes, $opt_jfb_ask_perms_location, $opt_jfb_ask_perms_groups, $opt_jfb_ask_perms_websites, $opt_jfb_ask_perms_work, $opt_jfb_ask_perms_customfreinds;
     global $jfb_name, $jfb_version, $opt_jfb_valid, $jfb_nonce_name;
